...
extern void _mlir_ciface_kernel(void*,void*,void*,void*,long);
int main(int argc,char**argv){
  ...
  double*x=malloc(8L*N);
  double*y=malloc(8L*N);
  double*T=malloc(8L*N);
  for(long i=0;i<N*N;i++){R;A[i]=U;}
  for(long i=0;i<N;i++){R;x[i]=U;}
  ...
  MR1 dy=mk1(y,N);
  MR1 dT=mk1(T,N);
  _mlir_ciface_kernel(&dA,&dx,&dy,&dT,N);
  volatile double sink=y[0];(void)sink;
  if(argc>2){FILE*f=fopen(argv[2],"wb");fwrite(y,8,N,f);fclose(f);}
  return 0;
}
