# Kernel `syrk` — optimize an MLIR affine loop nest for native performance

## What it computes
syrk: C = 0.9*C + A*A^T, LOWER triangle only (j<=i). A,C are N x N.
...
## Tested on THREE size regimes — you may submit one or three versions
  ...
  - **large**: N in [1152, 1664] (tested at an undisclosed N in this range)
