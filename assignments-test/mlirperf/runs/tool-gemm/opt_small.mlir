module {
  ...
  // Tiles: iT=64 kT=384 jT=384. UJ covers i<floor4(N); bottom rows separate.
  func.func @kernel(%A: memref<?x?xf64>, %B: memref<?x?xf64>, %C: memref<?x?xf64>, %N: index) attributes {llvm.emit_c_interface} {
    ...
    affine.for %ii = 0 to affine_map<()[s0] -> (s0 floordiv 4 * 4)>()[%N] step 64 {
      affine.for %kk = 0 to %N step 384 {
        affine.for %jj = 0 to %N step 384 {
          affine.for %i = affine_map<(d0) -> (d0)>(%ii)
              to min affine_map<(d0)[s0] -> (d0 + 64, s0 floordiv 4 * 4)>(%ii)[%N] step 4 {
            affine.for %k = affine_map<(d0) -> (d0)>(%kk) to min affine_map<(d0)[s0] -> (d0 + 384, s0)>(%kk)[%N] {
              ...
              affine.for %j = affine_map<(d0) -> (d0)>(%jj) to min affine_map<(d0)[s0] -> (d0 + 384, s0)>(%jj)[%N] {
