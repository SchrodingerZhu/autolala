# Kernel `mvt` — optimize an MLIR affine loop nest for native performance

## What it computes
mvt: x1 += A*y1 ; x2 += A^T*y2   (A is N x N; second pass reads A transposed).
...
## Tested on THREE size regimes — you may submit one or three versions
  - **small**: N in [1024, 2048] (tested at an undisclosed N in this range)
  - **medium**: N in [3072, 5120] (tested at an undisclosed N in this range)
  - **large**: N in [6144, 8192] (tested at an undisclosed N in this range)
