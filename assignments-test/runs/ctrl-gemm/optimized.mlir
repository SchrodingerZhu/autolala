module {
  func.func @kernel_gemm(%C: memref<?x?xf64>,
      ...
      %alpha = arith.constant 1.0 : f64
        ...
        // C[i][j] *= beta   (tiled over i,j for streaming locality)
        ...
        // Tiled GEMM: C[i][j] += alpha * A[i][k] * B[k][j]
        // Tile order ii, jj, kk -> i, k, j so a 32x32 block of B and C
        // stays resident and is reused across the k/i sweeps of the tile.
        affine.for %ii = 0 to %NI step 32 {
          affine.for %jj = 0 to %NJ step 32 {
