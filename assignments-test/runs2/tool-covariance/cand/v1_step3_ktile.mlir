module {
    func.func @kernel_covariance(%data: memref<?x?xf64>, %cov: memref<?x?xf64>, %mean: memref<?xf64>, %m: index, %n: index, %float_n: f64) {
        affine.for %loop_once = 0 to 1 {
            ...
            // Step 3: covariance, with i tiled by 32 so the i-tile columns of data are reused across j
            %float_n_minus_1 = arith.subf %float_n, %c1 : f64
            affine.for %ii = 0 to %m step 32 {
                affine.for %i = 0 to 32 {
                    affine.for %j = affine_map<(d0) -> (d0)> (%i + %ii) to %m {
                        affine.store %c0, %cov[%i + %ii, %j] : memref<?x?xf64>
                        affine.for %k = 0 to %n {
                            %cov_ij = affine.load %cov[%i + %ii, %j] : memref<?x?xf64>
                            %data_ki = affine.load %data[%k, %i + %ii] : memref<?x?xf64>
                        ...
                        %cov_sum = affine.load %cov[%i + %ii, %j] : memref<?x?xf64>
                        %cov_normalized = arith.divf %cov_sum, %float_n_minus_1 : f64
                        affine.store %cov_normalized, %cov[%i + %ii, %j] : memref<?x?xf64>
                        %cov_ij_final = affine.load %cov[%i + %ii, %j] : memref<?x?xf64>
                        affine.store %cov_ij_final, %cov[%j, %i + %ii] : memref<?x?xf64>
                    }
                }
            }
        } {dmd.extract}
