...
int main(int argc,char**argv){
  ...
  double* A=malloc(sizeof(double)*(long)N*N*N);
  double* C4=malloc(sizeof(double)*(long)N*N);
  double* sum=malloc(sizeof(double)*(long)N);
  ...
  volatile double sink = A[0]; (void)sink;
  if(argc>2){ FILE*f=fopen(argv[2],"wb");
    fwrite(A,sizeof(double),(long)N*N*N,f);
    fclose(f); }
  return 0;
}
